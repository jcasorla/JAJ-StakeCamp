...
namespace StakeCamp
{
    public partial class medicationCamperReport : Form
    {
        public medicationCamperReport()
        {
            InitializeComponent();
        }
        ...
        //*******************CAMPER MEDICATION QUERY*************************************
        private void medicationCamperReport_Load(object sender, EventArgs e)
        {
            // TODO: This line of code loads data into the 'girlsCampDataSet.adults' table. You can move, or remove it, as needed.

            Reports.sql = "SELECT customers.First_name, customers.Last_name, customers.Address, " +
                "customers.Home_Phone, customers.Cell_Phone, customers.Birth_Date, girls_medical.MedsYN, levels.Level, wards.Ward, nests.Nest_name, customers.Email " +
            "FROM nests " +
            "INNER JOIN ((wards INNER JOIN (levels INNER JOIN ((customers INNER JOIN girls_medical ON customers.CustomerID = girls_medical.customer_id) " +
            "INNER JOIN customer_level_link ON customers.CustomerID = customer_level_link.Customer_id) " +
        ...
        //*******************************************************************************
        private void medcamperbtn_Click(object sender, EventArgs e)
        {
